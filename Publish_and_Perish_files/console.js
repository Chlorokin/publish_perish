...
async function  consoleAddText(txt,speed_ms,hide_clickers){
    console.assert(hide_clickers === undefined || typeof hide_clickers === "boolean");
...
function createTyperObject(game_state){
    ...
    object.RemoveTyper = () => {
        ...
        document.getElementById("body_main").removeEventListener("keyup", object.typingFast);
        object.allow_typing = false;
    }

    object.typingSlow = () => {
        let game_state  = object.game_state;
        typing(game_state,object);
    }
    object.typingFast = () => {
        let game_state  = object.game_state;
        console.assert(game_state !== undefined, "game_state is undefined");
        for (let i = 0; i < 10; i++) {
            ...
            console.assert(typeof remove_function === "function", "remove_function is not a function");
            typing(game_state,object);
        }
    }
    object.AddTyper = (fast_or_slow) => {
        ...
        let pending_paper_name  = object.override_paper_title ? object.override_paper_title : genPaperName();
        game_state.pending_paper = pending_paper_name;
        typer_title.setAttribute('paper_name',pending_paper_name);
        if (fast_or_slow === "slow"){
            document.getElementById("body_main").addEventListener("keyup", object.typingSlow);
        }
        else{
            document.getElementById("body_main").addEventListener("keyup", object.typingFast);
        }
    }
    return object;
}
...
async function typing(game_state,typer_object){
    ...
    console.assert(game_state !== undefined, "game_state is undefined");
    console.assert(typeof remove_function !== "undefined", "remove_function is undefined");
    console.assert(typeof remove_function === "function", "remove_function is not a function");
    console.assert(typeof allow_typing === "boolean", "allow_typing is not a boolean"); 
    console.assert(typeof finished_function === "undefined" || typeof finished_function === "function", "finished_function is not a function");
    if (allow_typing === false){
    //await breaks the remove function
    //this is a horrible hack to get around this
        remove_function();
        return;
    }
