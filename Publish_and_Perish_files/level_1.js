...
async function levelOne(game_state){
  ...
  let task_object = level_object.GetTasksObject();
  ...
  let test_func = () => {alert('test')};
  level_loop_object.AddButtonObject("Test",test_func);
}


async function levelOne_old(game_state) {
  ...
  const listTasksClick = async () => {
    changeDisplayAllClickers('none')
    await listTasks(game_state);
  ...
  //play_area_div.appendChild(createButton("Long-term goals", "show_jobs"));
  //document.getElementById("show_jobs").addEventListener("click", showJobsClick);
